...
df <- read.csv('df_17_21_noclean.csv') %>%
  dplyr::select(year, country, total_earnings, # orden de datos panel
         ...
         pbicap, gdp_gr,
         CPI, # corrupcion
         internet, elect_acc, exp_tech, # tech access
         age_work,life_exp, # edades
         poblacion# people
        ) %>%
...
```{r}
df_standar <- df %>% 
  mutate(across(c("total_earnings", "total_players", "pbicap", "age_work", "poblacion"), ~log(.) %>% as.vector))
```
...
```{r}
pairs.panels(df_standar%>%dplyr::select(total_earnings, total_players, pbicap, internet,CPI),
...
X <- cbind(total_players, pbicap, gdp_gr, CPI, internet,
           elect_acc, exp_tech, age_work, life_exp, poblacion)
